	...
	try {
		const resp = await fetch('https://api.acchymns.app/discover', {
			...
			headers: {
				...
				'Authorization': 'Bearer GETTOKENFROMCHRIS',
			},
			body: JSON.stringify({
				'prompt': search_query.value,
				'allowed_hymnals': search_params.value.bookFilters,
				'search_type': 'both',
				'top_k': 10
			}),
		})
...
<template>
	...
	<h2 v-if="display_results" style="margin-top: 10px; margin-bottom: 10px">Top Results ({{ top_results.length }})</h2>
	<span v-if="error" style="margin-left: 30px; color: salmon">{{error}}</span>
	<div class="songlist">
        <RouterLink
            v-for="song in top_results"
	...
	<div class="content-footer" :style="{ marginBottom: 80 + keyboard_height + 'px' }">
		<div v-if="search_query.length == 0 && !display_results" class="suggestions-wrapper">
		...
		<div class="prompt-input-wrapper" :class="{ 'prompt-input-wrapper-outline': search_query.length > 0 }">
			...
			<div class="prompt-menu">
				...
				/>
				<button v-if="search_query.length > 0" :disabled="loading" class="submit-prompt" @click="handleSubmit()">
