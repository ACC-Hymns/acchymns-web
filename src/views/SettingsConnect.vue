...
let show_book = ref<boolean>(false);
...
async function populate_bg_color() {
    ...
    show_book.value = church_data.SHOW_BOOK.BOOL;
}

async function clear() {
    await set(request_client(), selected_church.value, "", "", [-1], "", show_clock.value, show_book.value);
}
...
async function set_book() {
    await set_show_book(request_client(), selected_church.value, show_book.value);
}
...
onMounted(async () => {
    ...
    book_sources.value = temp.filter(
        b => b.status == BookSourceType.DOWNLOADED || b.status == BookSourceType.IMPORTED || b.status == BookSourceType.BUNDLED,
    );
...
async function broadcast_reading() {
    ...
    await set(request_client(), selected_church.value, top_text, "BIBLE", [], bottom_text, show_clock.value, show_book.value);
}
...
async function broadcast_song_number() {
    ...
    await set(request_client(), selected_church.value, number, book.name?.medium || "", verses.value, book.primaryColor || "#000000", show_clock.value, show_book.value);
}
        ...
        <div v-else-if="status == UserStatus.Authorized" class="center-container">
            ...
            <div class="settings width-100">
                ...
                <a class="settings-option">
                    ...
                    <label class="switch">
                    <input v-model="show_book" type="checkbox" @change="set_book"/>
                        <span class="slider round"></span>
                    </label>
                </a>
