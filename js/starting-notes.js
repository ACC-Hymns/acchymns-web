...
function finished() {
    const sampler = new Tone.Sampler({
        ...
        onload: () => {
            var interval =
                window.localStorage.getItem("playbackInterval") == undefined
                    ? 0.25
                    : parseFloat(window.localStorage.getItem("playbackInterval"));
            var duration =
                window.localStorage.getItem("playbackDuration") == undefined
                    ? 3
                    : parseFloat(window.localStorage.getItem("playbackDuration"));
...
startingNotesButton.addEventListener("mousedown", async (_) => {
    ...
    for (var i = 0; i < index.length; i++) {
        ...
        request.onload = function () {
            ...
            if (this.customdata.iteration == this.customdata.index.length - 1) finished();
        };
        request.send();
    }
});
